...
const LIST_PATH = "/portal/participation/praise";
...
async function setupBrowser(): Promise<Browser> {
  ...
  if (process.env.PUPPETEER_EXECUTABLE_PATH) {
    ...
    logger.info("[fetchPraise] Using PUPPETEER_EXECUTABLE_PATH", { executablePath });
  } else {
    ...
    try {
      ...
      if (pptrPath && fs.existsSync(pptrPath)) {
        ...
        logger.info("[fetchPraise] Using puppeteer.executablePath()", { executablePath });
      }
    } catch (e) {
      logger.info("[fetchPraise] puppeteer.executablePath() not available, will install Chrome", { err: String(e) });
    }
      ...
      try {
        ...
        if (chromeBrowser) {
          ...
          if (resolved && fs.existsSync(resolved)) {
            ...
            logger.info("[fetchPraise] Using Chrome from @puppeteer/browsers", { executablePath, cacheDir });
          }
        }
      } catch (err) {
        logger.warn("[fetchPraise] Chrome install/setup failed", { err: String(err) });
      }
    }
  }
  ...
  logger.info("[fetchPraise] Launching browser");
...
async function fetchHtmlWithBrowser(browser: Browser, url: string): Promise<string> {
  ...
  try {
    ...
    logger.info("[fetchPraise] Fetching HTML", { url });
    ...
    if (!response || !response.ok()) {
      logger.warn("[fetchPraise] HTML fetch failed", { url, status });
      return "";
    }
    const html = await page.content();
    logger.info("[fetchPraise] HTML fetched", { url, status, length: html.length });
    return html;
  } finally {
    if (page) {
      try {
...
async function fetchDetailHtml(browser: Browser, detailUrl: string): Promise<string> {
  ...
  try {
    ...
    await page.setViewport({ width: 1920, height: 1080 });
    await page.setUserAgent(USER_AGENT);
    logger.info("[fetchPraise] Fetching detail", { url: detailUrl });
    ...
    const status = response?.status();
    if (!response || !response.ok()) {
      logger.warn("[fetchPraise] Detail fetch failed", { url: detailUrl, status });
      return "";
    }
    const html = await page.content();
    logger.info("[fetchPraise] Detail fetched", { url: detailUrl, status, length: html.length });
    return html;
  } finally {
    if (page) {
      try {
...
/**
 * 상세 HTML에서 Article 추출 (type: "praise")
 */
...
/**
 * 칭찬합시다 목록·상세 크롤링 후 articles 컬렉션에 저장 (type: 'praise')
 * Puppeteer 사용 (TLS/인증서 이슈 회피)
 * @param options.maxListPages 최대 목록 페이지 수 (기본 1)
 */
export async function fetchPraise(options?: { maxListPages?: number }): Promise<{
  ...
  try {
    for (let page = 1; page <= maxListPages; page++) {
      ...
      logger.info("[fetchPraise] List parsed", { page, count: listRows.length });
      if (listRows.length === 0) break;

      for (const row of listRows) {
        const detailUrl = `${BASE_URL}${LIST_PATH}?articleSeq=${row.articleSeq}`;
        logger.info("[fetchPraise] Article detail", { articleSeq: row.articleSeq, title: row.title });
        const existingByUrl = await articlesRef.where("url", "==", detailUrl).limit(1).get();
        if (!existingByUrl.empty) {
          logger.info("[fetchPraise] Duplicate by url, skip before detail fetch", {
        ...
        if (!detailHtml) {
          logger.warn("[fetchPraise] Detail HTML empty, skip", {
            articleSeq: row.articleSeq,
            title: row.title,
            url: detailUrl,
          });
        ...
        if (!article) {
          logger.warn("[fetchPraise] Detail parse failed, skip", {
            articleSeq: row.articleSeq,
            title: row.title,
            url: detailUrl,
          });
        ...
        articlesCount++;
        logger.info("[fetchPraise] Article saved", {
          articleSeq: row.articleSeq,
          title: article.title,
    ...
    logger.info("[fetchPraise] Done", { articlesCount });
  } finally {
    try {
      await browser.close();
      logger.info("[fetchPraise] Browser closed");
    } catch (e) {
      logger.warn("[fetchPraise] Browser close error", { err: String(e) });
    }
  }
