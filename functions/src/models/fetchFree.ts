...
const LIST_PATH = "/portal/participation/board";
...
async function setupBrowser(): Promise<Browser> {
  ...
  if (process.env.PUPPETEER_EXECUTABLE_PATH) {
    ...
    logger.info("[fetchFree] Using PUPPETEER_EXECUTABLE_PATH", { executablePath });
  } else {
    ...
    try {
      ...
      if (pptrPath && fs.existsSync(pptrPath)) {
        ...
        logger.info("[fetchFree] Using puppeteer.executablePath()", { executablePath });
      }
    } catch (e) {
      logger.info("[fetchFree] puppeteer.executablePath() not available, will install Chrome", { err: String(e) });
    }
      ...
      try {
        ...
        if (chromeBrowser) {
          ...
          if (resolved && fs.existsSync(resolved)) {
            ...
            logger.info("[fetchFree] Using Chrome from @puppeteer/browsers", { executablePath, cacheDir });
          }
        }
      } catch (err) {
        logger.warn("[fetchFree] Chrome install/setup failed", { err: String(err) });
      }
    }
  }
  ...
  logger.info("[fetchFree] Launching browser");
...
async function fetchHtmlWithBrowser(browser: Browser, url: string): Promise<string> {
  ...
  try {
    ...
    logger.info("[fetchFree] Fetching HTML", { url });
    ...
    if (!response || !response.ok()) {
      logger.warn("[fetchFree] HTML fetch failed", { url, status });
      return "";
    }
    const html = await page.content();
    logger.info("[fetchFree] HTML fetched", { url, status, length: html.length });
    return html;
  } finally {
    if (page) {
      try {
...
async function fetchDetailHtml(browser: Browser, detailUrl: string): Promise<string> {
  ...
  try {
    ...
    await page.setViewport({ width: 1920, height: 1080 });
    await page.setUserAgent(USER_AGENT);
    logger.info("[fetchFree] Fetching detail", { url: detailUrl });
    ...
    const status = response?.status();
    if (!response || !response.ok()) {
      logger.warn("[fetchFree] Detail fetch failed", { url: detailUrl, status });
      return "";
    }
    const html = await page.content();
    logger.info("[fetchFree] Detail fetched", { url: detailUrl, status, length: html.length });
    return html;
  } finally {
    if (page) {
      try {
...
/**
 * 상세 HTML에서 Article 추출 (type: "free")
 */
...
/**
 * 자유게시판 목록·상세 크롤링 후 articles 컬렉션에 저장 (type: 'free')
 */
export async function fetchFree(options?: { maxListPages?: number }): Promise<{
  ...
  try {
    for (let page = 1; page <= maxListPages; page++) {
      ...
      logger.info("[fetchFree] List parsed", { page, count: listRows.length });
      if (listRows.length === 0) break;

      for (const row of listRows) {
        const detailUrl = `${BASE_URL}${LIST_PATH}?articleSeq=${row.articleSeq}`;
        logger.info("[fetchFree] Article detail", { articleSeq: row.articleSeq, title: row.title });
        const existingByUrl = await articlesRef.where("url", "==", detailUrl).limit(1).get();
        if (!existingByUrl.empty) {
          logger.info("[fetchFree] Duplicate by url, skip before detail fetch", {
        ...
        if (!detailHtml) {
          logger.warn("[fetchFree] Detail HTML empty, skip", {
            articleSeq: row.articleSeq,
            title: row.title,
            url: detailUrl,
          });
        ...
        if (!article) {
          logger.warn("[fetchFree] Detail parse failed, skip", {
            articleSeq: row.articleSeq,
            title: row.title,
            url: detailUrl,
          });
    ...
    logger.info("[fetchFree] Done", { articlesCount });
  } finally {
    try {
      await browser.close();
      logger.info("[fetchFree] Browser closed");
    } catch (e) {
      logger.warn("[fetchFree] Browser close error", { err: String(e) });
    }
  }
