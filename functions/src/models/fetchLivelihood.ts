...
import { absoluteUrlsInHtml } from "../utils/absoluteUrlsInHtml.js";
import { truncateToMaxBytes } from "../utils/truncateBytes.js";

const BASE_URL = "https://www.inje.go.kr";
const LIST_PATH = "/portal/participation/livelihood";
...
async function setupBrowser(): Promise<Browser> {
  ...
  if (process.env.PUPPETEER_EXECUTABLE_PATH) {
    ...
    logger.info("[fetchLivelihood] Using PUPPETEER_EXECUTABLE_PATH", { executablePath });
  } else {
    ...
    try {
      ...
      if (pptrPath && fs.existsSync(pptrPath)) {
        ...
        logger.info("[fetchLivelihood] Using puppeteer.executablePath()", { executablePath });
      }
    } catch (e) {
      logger.info("[fetchLivelihood] puppeteer.executablePath() not available, will install Chrome", { err: String(e) });
    }
      ...
      try {
        ...
        if (chromeBrowser) {
          ...
          if (resolved && fs.existsSync(resolved)) {
            ...
            logger.info("[fetchLivelihood] Using Chrome from @puppeteer/browsers", { executablePath, cacheDir });
          }
        }
      } catch (err) {
        logger.warn("[fetchLivelihood] Chrome install/setup failed", { err: String(err) });
      }
    }
  }
  ...
  logger.info("[fetchLivelihood] Launching browser");
...
async function fetchHtmlWithBrowser(browser: Browser, url: string): Promise<string> {
  ...
  try {
    ...
    logger.info("[fetchLivelihood] Fetching HTML", { url });
    ...
    if (!response || !response.ok()) {
      logger.warn("[fetchLivelihood] HTML fetch failed", { url, status });
      return "";
    }
    const html = await page.content();
    logger.info("[fetchLivelihood] HTML fetched", { url, status, length: html.length });
    return html;
  } finally {
    if (page) {
      try {
...
async function fetchDetailHtml(browser: Browser, detailUrl: string): Promise<string> {
  ...
  try {
    ...
    await page.setViewport({ width: 1920, height: 1080 });
    await page.setUserAgent(USER_AGENT);
    logger.info("[fetchLivelihood] Fetching detail", { url: detailUrl });
    ...
    const status = response?.status();
    if (!response || !response.ok()) {
      logger.warn("[fetchLivelihood] Detail fetch failed", { url: detailUrl, status });
      return "";
    }
    const html = await page.content();
    logger.info("[fetchLivelihood] Detail fetched", { url: detailUrl, status, length: html.length });
    return html;
  } finally {
    if (page) {
      try {
...
/**
 * 상세 HTML에서 Article 추출 (type: "livelihood")
 */
...
/**
 * 생활장터 목록·상세 크롤링 후 articles 컬렉션에 저장 (type: 'livelihood')
 */
export async function fetchLivelihood(options?: { maxListPages?: number }): Promise<{
  ...
  logger.info("[fetchLivelihood] Start", { maxListPages });
  const browser = await setupBrowser();
  try {
    for (let page = 1; page <= maxListPages; page++) {
      const listUrl = page === 1 ? LIST_URL : `${LIST_URL}?pageIndex=${page}`;
      logger.info("[fetchLivelihood] List page", { page, listUrl });
      ...
      logger.info("[fetchLivelihood] List parsed", { page, count: listRows.length });
      if (listRows.length === 0) break;

      for (const row of listRows) {
        const detailUrl = `${BASE_URL}${LIST_PATH}?articleSeq=${row.articleSeq}`;
        logger.info("[fetchLivelihood] Article detail", { articleSeq: row.articleSeq, title: row.title });
        const existingByUrl = await articlesRef.where("url", "==", detailUrl).limit(1).get();
        if (!existingByUrl.empty) {
          logger.info("[fetchLivelihood] Duplicate by url, skip before detail fetch", {
        ...
        if (!detailHtml) {
          logger.warn("[fetchLivelihood] Detail HTML empty, skip", {
            articleSeq: row.articleSeq,
            title: row.title,
            url: detailUrl,
          });
        ...
        if (!article) {
          logger.warn("[fetchLivelihood] Detail parse failed, skip", {
            articleSeq: row.articleSeq,
            title: row.title,
            url: detailUrl,
          });
        ...
        articlesCount++;
        logger.info("[fetchLivelihood] Article saved", {
          articleSeq: row.articleSeq,
          title: article.title,
    ...
    logger.info("[fetchLivelihood] Done", { articlesCount });
  } finally {
    try {
      await browser.close();
      logger.info("[fetchLivelihood] Browser closed");
    } catch (e) {
      logger.warn("[fetchLivelihood] Browser close error", { err: String(e) });
    }
  }
