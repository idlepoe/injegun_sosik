...
/// Firestore articles 컬렉션 중 type=='praise' GET 전용 (페이징)
class PraiseRepository implements ArticleRepository {
  PraiseRepository({FirebaseFirestore? firestore})
  ...
  Future<PraisePageResult> getPage({
    int pageSize = 20,
    DocumentSnapshot<Map<String, dynamic>>? startAfter,
  }) async {
    var query = _firestore
        .collection('articles')
        .where('type', isEqualTo: 'praise')
    ...
    return PraisePageResult(items, lastDoc);
  }
}

/// 한 페이지 조회 결과 (리스트 + 커서)
class PraisePageResult {
  PraisePageResult(this.items, this.lastDoc);
